...
openssl genpkey -algorithm RSA -out "$dir/$domain.key" -config openssl-distinguished-name.cnf
openssl req -new -key "$dir/$domain.key" -out "$dir/$domain.csr" -config openssl-distinguished-name.cnf
