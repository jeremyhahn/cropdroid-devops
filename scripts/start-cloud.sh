...
tmux split-window -v
tmux send-keys -t CropCloud "./cropdroid-x64 cloud --debug --config-dir ./test --data-dir ./db/cloud --log-dir /var/log/cropdroid/cloud --keys keys --ssl=false --port 9093 --node-id 3 --datastore cockroach --enable-registrations" Enter
...
tmux split-window -v
tmux send-keys -t CropCloudLogs "tail -f /var/log/cropdroid/cloud/node-3.log" Enter
